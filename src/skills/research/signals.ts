...
export type DurableInstructionProposal = {
  skillName: string;
  description: string;
  content: string;
  goal: string;
...
function matchExistingSkill(
  ...
  return bestScore >= SKILL_MATCH_MIN_SCORE && bestScoreMatches === 1 ? best : undefined;
}
...
function buildInstructionGroup(params: {
  ...
  label: string;
  instructions: string[];
}): DurableInstructionProposal | undefined {
...
/** Extracts durable correction text without doing workspace skill discovery or routing. */
export function extractDurableInstructions(params: { messages: unknown[] }): string[] {
  ...
  for (const entry of transcript) {
    ...
    const instruction = extractInstruction(entry.text);
...
/**
 * Routes already-extracted durable instructions into one proposal per target skill.
 */
export function groupDurableInstructionProposals(params: {
  instructions: readonly string[];
  existingSkills?: readonly WorkspaceSkillSummary[];
  maxProposals?: number;
}): DurableInstructionProposal[] {
  const existingSkills = params.existingSkills ?? [];
  const existingSkillNameCounts = new Map<string, number>();
  for (const skill of existingSkills) {
    const skillName = normalizeSkillIndexName(skill.name);
    if (skillName) {
      existingSkillNameCounts.set(skillName, (existingSkillNameCounts.get(skillName) ?? 0) + 1);
    }
  }
  const uniquelyNamedSkills = existingSkills.filter(
    (skill) => existingSkillNameCounts.get(normalizeSkillIndexName(skill.name)) === 1,
  );
  const captured = params.instructions.slice(-MAX_CAPTURED_INSTRUCTIONS);
  if (captured.length === 0) {
    return [];
  }

  const groups = new Map<string, { title: string; label: string; instructions: string[] }>();
  for (const instruction of captured) {
    const existing = matchExistingSkill(instruction, uniquelyNamedSkills);
    const topic = existing
      ...
      : inferTopic(instruction);
    const skillName = normalizeSkillIndexName(topic.skillName);
    if (!skillName) {
      continue;
    }
    if ((existingSkillNameCounts.get(skillName) ?? 0) > 1) {
      continue;
    }
  ...
  const proposals: DurableInstructionProposal[] = [];
...
/**
 * Extracts candidate durable instructions from transcript text, grouped into one proposal per
 * target skill. Corrections route to an existing workspace skill when the vocabulary overlaps;
 * otherwise they fall back to a conservative inferred topic.
 */
export function extractDurableInstructionProposals(params: {
  ...
  return groupDurableInstructionProposals({
    instructions: extractDurableInstructions({ messages: params.messages }),
    existingSkills: params.existingSkills,
    maxProposals: params.maxProposals,
  });
}

/** Rebuilds an auto-capture proposal with earlier and current-turn corrections. */
export function mergeDurableInstructionProposal(
  proposal: DurableInstructionProposal,
  previousInstructions: readonly string[],
): DurableInstructionProposal {
  const instructions = [...previousInstructions];
