...
describe("extractDurableInstructionProposals", () => {
  ...
  it("falls back to topic inference when existing skills tie", () => {
    const proposals = extractDurableInstructionProposals({
      messages: [
        userMessage("I don't want to repeat myself: review the source material before answering."),
      ],
      existingSkills: [{ name: "code-review" }, { name: "content-review" }],
    });
    expect(proposals).toHaveLength(1);
    expect(proposals[0].skillName).toBe("learned-workflows");
  });

  it("groups equivalent raw and inferred skill names into one proposal", () => {
    const proposals = extractDurableInstructionProposals({
      messages: [
        userMessage("I don't want to repeat myself: stop using the old workflows when drafting."),
        userMessage("You're still using transcripts as tone references in every draft."),
      ],
      existingSkills: [{ name: "Learned Workflows" }],
    });
    expect(proposals).toHaveLength(1);
    expect(proposals[0].skillName).toBe("learned-workflows");
    expect(proposals[0].content).toContain("old workflows");
    expect(proposals[0].content).toContain("transcripts as tone references");
  });

  it("does not route to ambiguous normalized existing skill names", () => {
    const proposals = extractDurableInstructionProposals({
      messages: [
        userMessage(
          "I don't want to repeat myself: validate every source before publishing the draft.",
        ),
      ],
      existingSkills: [
        { name: "Signal Scout", description: "Validate sources before publishing." },
        { name: "signal-scout", description: "Collect market signals before drafting." },
      ],
    });
    expect(proposals).toHaveLength(1);
    expect(proposals[0].skillName).toBe("learned-workflows");
